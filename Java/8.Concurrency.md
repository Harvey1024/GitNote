...
通过 volatile 关键字修饰变量，保证不同线程对共享变量操作时的可见性。
对方法或代码块加锁使用 synchronized 关键字，实现互斥

Thread 线程阻塞和唤醒

- sleep 时间一到自己醒来，休眠，需要使用 try catch 捕获 interruptedException 异常
- wait/notify/notifyAll 等待，需要别人唤醒
- join 等待另外一个线程结束
- interrupt 向另外一个线程发送中断信号，该线程收到后会触发 InteruptiedException(可解除阻塞））并进行下一步处理
